...
/**
 * Service implementation layer defining the monthly customer statement
 * validation process and creating the final response JSON
 * 
 * @author Payel
 *
 */
...
public class CustomerServiceImpl implements CustomerService {
	...
	/**
	 * This method does the initial validation for the request payload that came in
	 * as part of the input to the service
	 * 
	...
	public PostProcessingResult processCustomerRecords(List<CustomerStatement> customerStatements,
		...
		// Logic to populate the response JSON in case the request payload was valid and
		// had no processing errors
	...
	/**
	 * Method to parse request payload, process the customer statement and generate
	 * the response JSON for error scenarios
	 * 
	 * @param customerStatements
	 * @param errorRecords
	 * @param finalResultPostProcessing
	...
	private PostProcessingResult processStatementForErroneousInputs(List<CustomerStatement> customerStatements,
		...
		for (CustomerStatement customerStatement : customerStatements) {
			...
			// Dummy scenario/logic to throw "Internal Server Error", retrieving the test
			// data from application.properties
			if (configUtility.getProperty(RETRIEVE_INTERNAL_SERVER_TEST_PROPERTY_DATA)
					.equals(customerStatement.getAccountNumber())) {
